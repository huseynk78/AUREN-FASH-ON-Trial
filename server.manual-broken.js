...
if (
  ...
  console.error(
    "ADMIN_PASSWORD must be set in a private .env file. See .env.example.",
  );
  process.exit(1);
}
...
const NotificationService = {
  record(orderId, status) {
    ...
    const order = db
      .prepare("SELECT id, customer_name, phone FROM orders WHERE id = ?")
      .get(orderId);
    const text = `Bildiriş göndəriləcək: sifariş #${order.id} → ${STATUS_LABELS[status]}`;
    db.prepare(
      `INSERT INTO notifications (order_id, status, channel, state, message, created_at)
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && route === "/thank-you")
      return thankYou(req, res, url);
    if (req.method === "GET" && route === "/health")
      return sendText(res, 200, "ok");
    if (req.method === "GET" && route === "/styles.css")
      return staticFile(res, "styles.css", "text/css; charset=utf-8");

    if (req.method === "GET" && route === "/admin/login")
      return loginForm(req, res, url);
    ...
    if (req.method === "POST" && route === "/admin/logout")
      return logout(req, res, session);
    if (req.method === "GET" && route === "/admin")
      return adminOrders(req, res, url, session);
    if (req.method === "POST" && /^\/admin\/orders\/\d+\/status$/.test(route))
      return updateOrderStatus(req, res, route, session);
    if (req.method === "GET" && route === "/admin/products")
      return productsPage(req, res, url, session);
    if (req.method === "POST" && route === "/admin/products")
      return saveProduct(req, res, session);
    if (req.method === "POST" && /^\/admin\/products\/\d+\/toggle$/.test(route))
      return toggleProduct(req, res, route, session);
    if (req.method === "GET" && route === "/admin/settings")
      return settingsPage(req, res, url, session);
    if (req.method === "POST" && route === "/admin/settings")
      return saveSettings(req, res, session);
    if (req.method === "GET" && route === "/admin/export.xls")
      return exportOrders(req, res);
    ...
    return sendHtml(
      ...
      page(
        "Xəta",
        '<main class="container"><h1>Gözlənilməyən xəta</h1><p>Zəhmət olmasa yenidən cəhd edin.</p></main>',
      ),
    );
  }
});
...
server.listen(PORT, () =>
  console.log(`Order manager is running at http://localhost:${PORT}`),
);
...
function setupDatabase() {
  ...
  const setting = db
    .prepare("SELECT value FROM settings WHERE key = 'exchange_rate'")
    .get();
  if (!setting)
    db.prepare(
      "INSERT INTO settings (key, value, updated_at) VALUES (?, ?, ?)",
    ).run("exchange_rate", "1.70", now());
  const row = db.prepare("SELECT COUNT(*) AS count FROM products").get();
  if (row.count === 0) {
    const insert = db.prepare(
      "INSERT INTO products (name, description, price_azn, is_active, created_at, updated_at) VALUES (?, ?, ?, 1, ?, ?)",
    );
    const timestamp = now();
    insert.run(
      "Klassik köynək",
      "Rahat, gündəlik model",
      35,
      timestamp,
      timestamp,
    );
...
function parseSizes(value) {
  try {
    const sizes = Array.isArray(value) ? value : JSON.parse(value || '[]');
...
function customerForm(req, res) {
  const products = db
    .prepare(
      "SELECT id, name, description, sizes, image_url, price_azn FROM products WHERE is_active = 1 ORDER BY name",
    )
    .all()
    .map((product) => ({
      ...product,
  ...
  if (products.length === 0)
    return sendHtml(
      ...
      page(
        "Məhsullar",
        '<main class="container"><h1>Tezliklə</h1><p>Hazırda sifariş üçün məhsul yoxdur.</p></main>',
      ),
    );
  ...
  function row(){const qty=document.createElement('input') const size=document.createElement('select');
size.className='size-select';
size.setAttribute('aria-label','Beden');; el.className='item-row';
    ...
    el.append(select, size, qty, price, remove);const update = () => {
  const p = product(select.value);

  size.replaceChildren();
  p.sizes.forEach((sizeValue) => {
  ...
  price.textContent = money(p.price_azn * Number(qty.value || 0)) + ' AZN';
  calculate();
};

select.addEventListener('change', update);
size.addEventListener('change', calculate);
qty.addEventListener('input', update); price.textContent=money(p.price_azn*Number(qty.value||0))+' AZN'; calculate()}; select.addEventListener('change',update); qty.addEventListener('input',update);
    remove.addEventListener('click',()=>{if(rows.children.length>1){el.remove();calculate()}}); rows.appendChild(el); update(); }
  function calculate(){let sum=0, orderItems=[]; rows.querySelectorAll('.item-row').forEach(el=>{const id=el.querySelector('.product-select').value, quantity=Math.max(1,Math.min(99,Number(el.querySelector('.qty-input').value)||1)), p=product(id); sum+=p.price_azn*quantity; orderItems.push({
  productId: p.id,
  quantity,
  size: el.querySelector('.size-select').value
});}); document.querySelector('#total-azn').textContent=money(sum)+' AZN'; document.querySelector('#total-usd').textContent='$'+money(sum/RATE)+' USD'; itemsInput.value=JSON.stringify(orderItems)}
...
async function createOrder(req, res) {
  ...
  let requestedItems;
  try {
    requestedItems = JSON.parse(body.items || "[]");
  } catch {
    requestedItems = [];
  }
  if (
    ...
    return sendHtml(
      ...
      errorPage(
        "Sifarişi yoxlayın",
        "Bütün məlumatları və ən az bir məhsulu daxil edin.",
      ),
    );
  }
  const chosen = [];
  for (const item of requestedItems) {
    ...
    if (
      ...
      return sendHtml(
        res,
        400,
        errorPage("Yanlış miqdar", "Miqdar 1–99 arasında olmalıdır."),
      );
    const product = db
      .prepare(
        "SELECT id, name, price_azn FROM products WHERE id = ? AND is_active = 1",
      )
      .get(id);
    if (!product)
      return sendHtml(
        ...
        errorPage(
          "Məhsul artıq yoxdur",
          "Seçdiyiniz məhsullardan biri artıq aktiv deyil. Səhifəni yeniləyib yenidən cəhd edin.",
        ),
      );
    chosen.push({ product, quantity, selectedSize });
  }
  const total = chosen.reduce(
    (sum, item) => sum + item.product.price_azn * item.quantity,
    0,
  );
  const rate = exchangeRate();
  const timestamp = now();
  ...
  try {
    const result = db
      .prepare(
        `INSERT INTO orders (customer_name, phone, address, status, total_azn, exchange_rate, total_usd, created_at, updated_at)
      VALUES (?, ?, ?, 'new', ?, ?, ?, ?, ?)`,
      )
      .run(
        name,
    ...
    const itemInsert = db.prepare(
      "INSERT INTO order_items (order_id, product_id, product_name, unit_price_azn, quantity, selected_size, line_total_azn) VALUES (?, ?, ?, ?, ?, ?, ?)",
    );
    chosen.forEach(({ product, quantity, selectedSize }) =>
      itemInsert.run(
        orderId,
        product.id,
        product.name,
        ...
        quantity,
        selectedSize,
        product.price_azn * quantity,
      ),
    );
    db.prepare(
      "INSERT INTO order_status_history (order_id, old_status, new_status, actor, created_at) VALUES (?, NULL, ?, ?, ?)",
    ).run(orderId, "new", "customer", timestamp);
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
...
function thankYou(req, res, url) {
  ...
  const order = db
    .prepare("SELECT id, total_azn, total_usd FROM orders WHERE id = ?")
    .get(id);
  ...
  return sendHtml(
    ...
    page(
      ...
      `<main class="container narrow"><section class="card centered success"><p class="success-mark">✓</p><h1>Sifarişiniz qəbul edildi</h1><p>Sifariş nömrəniz <strong>#${order.id}</strong>-dir.</p><p>Cəmi: <strong>${money(order.total_azn)} AZN</strong> <small>($${money(order.total_usd)} USD)</small></p><p>Sifarişin vəziyyəti dəyişdikdə sizinlə əlaqə saxlanılacaq.</p><a class="button primary" href="/">Yeni sifariş ver</a></section></main>`,
    ),
  );
}
...
function loginForm(req, res, url) {
  ...
  const message = failed
    ? '<p class="alert error">Şifrə düzgün deyil.</p>'
    : "";
  return sendHtml(
    res,
    200,
...
async function login(req, res) {
  ...
  const valid =
    supplied.length === ADMIN_PASSWORD.length &&
  ...
  res.setHeader(
    "Set-Cookie",
    `shop_session=${id}; HttpOnly; SameSite=Lax; Path=/; Max-Age=43200`,
  );
  redirect(res, "/admin");
}
...
function adminOrders(req, res, url, session) {
  ...
  const params = [];
  if (search) {
    filters.push("(o.customer_name LIKE ? OR o.phone LIKE ?)");
  ...
  const orders = db
    .prepare(
      `SELECT o.*, (SELECT GROUP_CONCAT(product_name || ' ×' || quantity, ', ') FROM order_items WHERE order_id = o.id) AS items FROM orders o ${where} ORDER BY o.created_at DESC`,
    )
    .all(...params);
  const notifications = db
    .prepare(
      "SELECT n.*, o.customer_name FROM notifications n JOIN orders o ON o.id=n.order_id ORDER BY n.created_at DESC LIMIT 15",
    )
    .all();
  const statusOptions = [
    ...
    ...VALID_STATUSES.map(
      (s) =>
        `<option value="${s}" ${status === s ? "selected" : ""}>${STATUS_LABELS[s]}</option>`,
    ),
  ].join("");
...
async function updateOrderStatus(req, res, route, session) {
  ...
  if (!VALID_STATUSES.includes(next))
    return sendHtml(
      res,
      400,
      errorPage("Yanlış status", "Bu status tanınmır."),
    );
  ...
  if (order.status !== next) {
    ...
    try {
      ...
      ).run(next, timestamp, id);
      db.prepare(
        "INSERT INTO order_status_history (order_id, old_status, new_status, actor, created_at) VALUES (?, ?, ?, ?, ?)",
      ).run(id, order.status, next, "admin", timestamp);
      NotificationService.record(id, next);
      db.exec("COMMIT");
    } catch (error) {
      db.exec("ROLLBACK");
      throw error;
...
function productsPage(req, res, url, session) {
  ...
  const current = editing
    ? db.prepare("SELECT * FROM products WHERE id = ?").get(editing)
    : null;
  const products = db
    .prepare("SELECT * FROM products ORDER BY is_active DESC, name")
    .all();
  const rows = products
    .map(
      (p) =>
        `<tr><td>${escapeHtml(p.name)}<br><small>${escapeHtml(p.description)}</small></td><td>${money(p.price_azn)} AZN</td><td><span class="badge ${p.is_active ? "active" : "inactive"}">${p.is_active ? "Aktiv" : "Gizli"}</span></td><td><a class="text-link" href="/admin/products?edit=${p.id}">Dəyiş</a><form method="post" action="/admin/products/${p.id}/toggle" class="inline-form"><input type="hidden" name="csrf" value="${session.csrf}"><button class="link-button">${p.is_active ? "Gizlət" : "Aktiv et"}</button></form></td></tr>`,
    )
    .join("");
  ...
  const content = adminLayout(
    "Məhsullar",
    "products",
    `<div class="two-column"><section class="card"><h2>${formTitle}</h2><form method="post" action="/admin/products"><input type="hidden" name="csrf" value="${session.csrf}"><input type="hidden" name="id" value="${current ? current.id : ""}"><label>Məhsul adı<input name="name" required maxlength="100" value="${escapeAttr(current ? current.name : "")}"></label>
    <label>Açıqlama<textarea name="description" maxlength="300" rows="3">${escapeHtml(current ? current.description : "")}</textarea></label>
    <label>Bedenler
  <input
    name="sizes"
    required
    maxlength="100"
    value="${escapeAttr(current ? parseSizes(current.sizes).join(", ") : "")}"
...
<label>Görsel bağlantısı
  <input
    ...
    type="url"
    maxlength="500"
    value="${escapeAttr(current ? current.image_url : "")}"
    placeholder="https://site.com/urun-gorseli.jpg"
  >
</label>
    <label>Qiymət (AZN)<input name="price_azn" type="number" min="0" step="0.01" required value="${current ? escapeAttr(current.price_azn) : ""}"></label>
    <button class="button primary submit">${current ? "Dəyişiklikləri yadda saxla" : "Məhsul əlavə et"}</button>${current ? '<a class="text-link" href="/admin/products">Ləğv et</a>' : ""}</form></section><section class="table-wrap"><table><thead><tr><th>Məhsul</th><th>Qiymət</th><th>Görünürlük</th><th></th></tr></thead><tbody>${rows}</tbody></table></section></div>`,
...
async function saveProduct(req, res, session) {
  ...
  const price = Number(body.price_azn);
  const sizes = [
    ...new Set(
      String(body.sizes || "")
        .split(",")
  ...
  if (!name || !Number.isFinite(price) || price < 0 || sizes.length === 0) {
    return sendHtml(
      res,
      400,
      errorPage(
        "Məhsulu yoxlayın",
        "Ad, ən az bir bədən və düzgün qiymət daxil edin.",
      ),
    );
  }
  ...
  if (imageUrl && !isValidImageUrl(imageUrl)) {
    return sendHtml(
      ...
      400,
      errorPage(
        "Görsel bağlantısını yoxlayın",
        "Görsel bağlantısı http:// və ya https:// ilə başlamalıdır.",
      ),
    );
  }
  const timestamp = now();
  if (id) {
    const product = db
      .prepare(
        "SELECT id, name, sizes, price_azn FROM products WHERE id = ? AND is_active = 1",
      )
      .get(id);
    const selectedSize = clean(item.size, 20);
    ...
    if (!availableSizes.includes(selectedSize)) {
      return sendHtml(
        ...
        errorPage(
          "Bədəni yoxlayın",
          "Seçilən bədən bu məhsul üçün mövcud deyil.",
        ),
      );
    }
    if (!product) return notFound(res);
    db.prepare(
      "UPDATE products SET name=?, description=?, sizes=?, image_url=?, price_azn=?, updated_at=? WHERE id=?",
    ).run(
      name,
      description,
    ...
    db.prepare(
      "INSERT INTO products (name, description, sizes, image_url, price_azn, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)",
    ).run(
      name,
      description,
      JSON.stringify(sizes),
...
async function toggleProduct(req, res, route, session) {
  ...
  if (!validCsrf(body, session)) return forbidden(res);
  const id = Number(route.split("/")[3]);
  const p = db.prepare("SELECT is_active FROM products WHERE id=?").get(id);
...
function settingsPage(req, res, url, session) {
  ...
  const rate = exchangeRate();
  const notice = saved
    ? '<p class="alert success-alert">Məzənnə yadda saxlanıldı.</p>'
    : "";
  const content = adminLayout(
    "Ayarlar",
    "settings",
...
async function saveSettings(req, res, session) {
  ...
  if (!Number.isFinite(rate) || rate <= 0 || rate > 100)
    return sendHtml(
      ...
      errorPage(
        "Məzənnəni yoxlayın",
        "0.01 ilə 100 arasında düzgün rəqəm yazın.",
      ),
    );
...
function exportOrders(req, res) {
  const orders = db
    .prepare(
      `SELECT o.*, GROUP_CONCAT(oi.product_name || ' x' || oi.quantity, ', ') AS items FROM orders o LEFT JOIN order_items oi ON oi.order_id=o.id GROUP BY o.id ORDER BY o.created_at DESC`,
    )
    .all();
  const header = [
    "Sifariş #",
  ...
  res.writeHead(200, {
    ...
    "Content-Disposition": 'attachment; filename="orders-export.xls"',
...
function adminLayout(title, active, body, session) {
  const nav = [
    ["orders", "/admin", "Sifarişlər"],
    ["products", "/admin/products", "Məhsullar"],
    ["settings", "/admin/settings", "Ayarlar"],
  ]
    .map(
      ([id, href, label]) =>
        `<a class="${active === id ? "selected" : ""}" href="${href}">${label}</a>`,
    )
    .join("");
...
async function logout(req, res, session) {
  ...
  res.setHeader(
    "Set-Cookie",
    "shop_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0",
  );
  redirect(res, "/admin/login");
}
...
function exchangeRate() {
  return Number(
    db.prepare("SELECT value FROM settings WHERE key = 'exchange_rate'").get()
      .value,
  );
}
function now() {
...
function page(title, body) {
  return `<!doctype html><html lang="az"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#24443a"><title>${escapeHtml(title)}</title><link rel="stylesheet" href="/styles.css"></head><body>${body}</body></html>`;
}
function errorPage(title, message) {
  return page(
    title,
    `<main class="container narrow"><section class="card centered"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p><a class="button primary" href="/">Geri qayıt</a></section></main>`,
  );
}
...
function forbidden(res) {
  return sendHtml(
    ...
    errorPage(
      "Giriş rədd edildi",
      "Təhlükəsizlik yoxlaması alınmadı. Səhifəni yeniləyin.",
    ),
  );
}
function notFound(res) {
  return sendHtml(
    ...
    page(
      "Tapılmadı",
      '<main class="container narrow"><section class="card centered"><h1>Səhifə tapılmadı</h1><a class="button primary" href="/">Ana səhifə</a></section></main>',
    ),
  );
}
